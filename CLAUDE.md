...
- **iOS-side deviations from the web are allowed and expected.** The PWA is the data-model source of truth, but iOS can choose different presentation when native idioms read better — e.g. native `Menu` / `.confirmationDialog` / SF Symbols / inline `Picker` instead of a web-style popover or bottom sheet. Always state the deviation and the reason in the PR description (and inline as a comment if it's load-bearing) so the cross-platform mental model stays in sync. **Append every intentional deviation to `docs/web-deviations.md`** — the `log-deviation` skill (in `.claude/skills/log-deviation/`) handles this proactively when shipping a divergent change. The ledger is what the team uses later to decide whether to port the iOS improvement back to the PWA.
